...
# Coincidentally, this guide will be centred around this class, how to create
# it and then some manipulations of the class.
...
# from a Uniform distribution. Much of echidna is built using the `numpy` and
# `SciPy` packages and we will use them here to generate the random numbers.
...
# numpy only prints the first three and last three entries. Since our energy
# range is in the middle, all our events are in the** `...` **part at the
# moment. But we will see entries printed out later when we apply some cuts.**

# ## Plotting

# Another useful way to inspect the `Spectra` created is to plot it. Support
# is available within echidna to plot using either `ROOT` or `matplotlib` and
...
# and "random smear". The "random smear" algorithm takes each event in each
# bin and randomly assigns it a new energy from the Gaussian distribution for
# that bin - it is fast but not very accurate for low statistics. The
# "weighted smear" algorithm is slower but much more accurate, as re-weights
# each bin by taking into account all other nearby bins within a pre-defined
# range. We will use the "weighted smear" method in this example.
...
# the spectrum too tightly before smearing as you may end up cutting bins that
# would have influenced the smearing. Cuts can be applied using the `shrink`
# method. (Confusingly there is also a `cut` method which is almost identical
# to the `shrink` method, but updates the number of events the spectrum
# represents, after the cut is applied. Unless you are sure this is what you
# want to do, it is probably better to use the `shrink` method.) To shrink
...
# We now have a nice smeared version of our original spectrum. To prepare the
# spectrum for a final analysis there are a few final manipulations we may
# wish to do.
...
# `scale` method should always be supplied with the number of events the full
# spectrum (i.e. before any cuts using `shrink` or `shrink_to_roi`) should
