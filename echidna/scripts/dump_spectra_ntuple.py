...
def read_and_dump_ntuple(fname, half_life, spectrum_name, save_path):
    """ Creates both mc and reco spectra from ROOT ntuple files, dumping the results as a
...
if __name__ == "__main__":
    ...
    if len(args) % 2 == 0:
        for i in range(0, len(args)/2+1, 2):
            ...
            read_and_dump_ntuple(fname, float(args[i+1]), spectrum_name, save_path)
    elif len(args) % 2 == 1:
    ...
    if options.Read_from_txt_file is not False:
        ...
        for idx, fname in enumerate(path_list):
            spectrum_name = fname[fname.rfind('/', 0, -1)+1:]
            read_and_dump_ntuple(path, half_life_list[idx], spectrum_name, save_path)
