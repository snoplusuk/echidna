...
class Spectra(object):
    ...
    def scale(self, num_decays):
        ...
        *self._num_decays* to *num_decays*. *self._num_decays* is updated
        ...
        """
        self._data = numpy.multiply(self._data, float(num_decays) / float(self._num_decays))
        self._num_decays = num_decays
    ...
    def rebin(self, *args):
        ...
        Raises:
          ValueError: Number of dimensions incorrect for spectra
          ValueError: Old bins/ New bins must be integer
        '''
        shape = self._data.shape
        len_shape = len(shape)
        if len_shape != len(args):
            raise ValueError("Number of dimensions incorrect")
